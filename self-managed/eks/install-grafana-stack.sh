...
helm install loki --values helm/loki.yaml grafana/loki-stack --namespace grafana && \
kubectl rollout status statefulset loki --namespace grafana --timeout=300s && \
helm install mimir --values helm/mimir.yaml grafana/mimir-distributed --namespace grafana && \
#kubectl rollout status statefulset mimir --namespace grafana --timeout=300s && \
#if mimir works, no need for prom too
...
helm install tempo --values helm/tempo.yaml grafana/tempo --namespace grafana && \
kubectl rollout status statefulset tempo --namespace grafana --timeout=300s && \
